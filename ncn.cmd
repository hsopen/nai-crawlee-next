...
powershell -Command "if (!(Test-Path 'tasksConfig')) { Write-Host '[提示] 创建 tasksConfig 目录...'; New-Item -ItemType Directory -Path 'tasksConfig' -Force }"
powershell -Command "if (!(Test-Path 'tasksConfig\tamplate.yaml')) { Write-Host '[提示] 创建 tamplate.yaml 文件...'; New-Item -ItemType File -Path 'tasksConfig\tamplate.yaml' -Force }"
powershell -Command "if (!(Test-Path 'tasksConfig\done')) { Write-Host '[提示] 创建 done 目录...'; New-Item -ItemType Directory -Path 'tasksConfig\done' -Force }"
powershell -Command "if (!(Test-Path 'tasksConfig\undone')) { Write-Host '[提示] 创建 undone 目录...'; New-Item -ItemType Directory -Path 'tasksConfig\undone' -Force }"
powershell -Command "if (!(Test-Path 'sitemap')) { Write-Host '[提示] 创建 sitemap 目录...'; New-Item -ItemType Directory -Path 'sitemap' -Force }"
powershell -Command "pnpm exec tsx ./dist/main.js"
pause
